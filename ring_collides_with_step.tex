...
\\
    ...
    \draw (ring.south) -- ++(3, 0) -- ++(0, 0.5) -- node(stepmiddle){} ++(3, 0) node (steprightedge){};
    \draw (ring.south) -- ++(-1cm, 0);
    % initial velocity vector
    \draw[force, ->] (ring.center) -- ++(1.5cm, 0) node[above]{$\vec{v}$};
    \draw[force, draw=black, ->] (ring.center) -- node[right] {$R$} ++(0, -1cm);

    \draw[force, draw=black, <->] (stepmiddle.center) -- node[right] {$h$} ++(0, -0.5);
